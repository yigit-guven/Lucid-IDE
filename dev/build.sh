...
export APP_NAME="LucidIDE"
export ASSETS_REPOSITORY="yigit-guven/Lucid-IDE"
export BINARY_NAME="lucid"
export CI_BUILD="no"
export GH_REPO_PATH="yigit-guven/Lucid-IDE"
export ORG_NAME="yigit-guven"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="yigit-guven/Lucid-IDE"
      export BINARY_NAME="lucid-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-lucidide
    else
      echo "{}" > ~/.gyp/include.gypi.pre-lucidide
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-lucidide ~/.gyp/include.gypi
  fi
