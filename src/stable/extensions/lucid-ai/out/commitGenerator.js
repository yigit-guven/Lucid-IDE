...
async function getRepository(git, uri) {
    ...
    if (uri) {
        ...
        const found = git.repositories.find((r) => r.rootUri.toString() === targetUri.toString());
        if (found) {
            return found;
        }
    }
    ...
    if (activeEditor) {
        ...
        const found = git.repositories.find((r) => docUri.fsPath.startsWith(r.rootUri.fsPath));
        if (found) {
            return found;
        }
    }
