...
const viewsContainerSchema: IJSONSchema = {
	...
	properties: {
		id: {
			description: localize({ key: 'vscode.extension.contributes.views.containers.id', comment: ['Contribution refers to those that an extension contributes to LucidIDE through an extension/contribution point. '] }, "Unique id used to identify the container in which views can be contributed using 'views' contribution point"),
			type: 'string',
			pattern: '^[a-zA-Z0-9_-]+$'
		},
		title: {
	...
	private registerCustomViewContainer(id: string, title: string, icon: URI | ThemeIcon, order: number, extensionId: ExtensionIdentifier | undefined, location: ViewContainerLocation): ViewContainer {
		...
		if (!viewContainer) {
			...
			viewContainer = this.viewContainersRegistry.registerViewContainer({
				...
				order,
				icon,
				openCommandActionDescriptor: id === 'workbench.view.extension.lucid-ai-sidebar' ? {
					id,
					title: { value: title, original: title },
