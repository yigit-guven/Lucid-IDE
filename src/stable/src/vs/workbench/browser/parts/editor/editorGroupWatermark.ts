...
interface WatermarkEntry {
	...
	readonly when?: {
		readonly native?: ContextKeyExpression;
		readonly web?: ContextKeyExpression;
	};
}
...
const toggleAIChat: WatermarkEntry = { text: localize('watermark.toggleAIChat', "Toggle AI Chat"), id: 'workbench.view.extension.lucid-ai-sidebar' };
...
export class EditorGroupWatermark extends Disposable {
	...
	private filterEntries(entries: WatermarkEntry[]): WatermarkEntry[] {
		const filteredEntries = entries
			...
			.filter(entry => entry.id === 'workbench.view.extension.lucid-ai-sidebar' || !!CommandsRegistry.getCommand(entry.id))
			.filter(entry => entry.id === 'workbench.view.extension.lucid-ai-sidebar' || !!this.keybindingService.lookupKeybinding(entry.id));
