...
export class ReleaseNotesManager extends Disposable {
	...
	private async getBase(useCurrentFile: boolean) {
		...
		return URI.parse('https://raw.githubusercontent.com/yigit-guven/Lucid-IDE/main');
	}
	...
	private async loadReleaseNotes(version: string, useCurrentFile: boolean): Promise<string> {
		const tagUrl = `https://api.github.com/repos/yigit-guven/Lucid-IDE/releases/tags/${version}`;
		const latestUrl = `https://api.github.com/repos/yigit-guven/Lucid-IDE/releases/latest`;
		...
		const fetchReleaseNotes = async () => {
			...
			try {
				if (useCurrentFile) {
					...
					let responseText;
					try {
						responseText = await asTextOrError(await this._requestService.request({ url: tagUrl, callSite: 'releaseNotesEditor.fetchReleaseNotes' }, CancellationToken.None));
					} catch {
					...
					if (text) {
						const title = releaseObj.name || releaseObj.tag_name || `Lucid IDE ${version}`;
						text = `# ${title}\n\n${text}`;
					}
				}
			} catch (e) {
				throw new Error('Failed to fetch release notes: ' + (e instanceof Error ? e.message : String(e)));
			}
